...
{
    ImGuiContext& g = *GImGui;
    const bool need_backup = (g.CurrentWindow->DC.TextWrapPos < 0.0f);  // Keep existing wrap posicion if one is already set
    if (need_backup)
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_visible_v, ImS64 size_contents_v, ImDrawFlags draw_rounding_flags)
{
    ...
    // Handle input right away. None of the code of Begin() is relying on scrolling posicion before calling Scrollbar().
    bool held = false;
    bool hovered = false;
    ...
    float grab_v_norm = scroll_ratio * (scrollbar_size_v - grab_h_pixels) / scrollbar_size_v; // Grab posicion in normalized space
    if (held && allow_interaction && grab_h_norm < 1.0f)
    {
        const float scrollbar_pos_v = bb.Min[axis];
        const float mouse_pos_v = g.IO.MousePos[axis];

        // Click posicion in scrollbar normalized space (0.0f->1.0f)
        ...
        // It is ok to modify Scroll here because we are being called in Begin() after the calculation of ContentSize and before setting up our starting posicion
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w)
{
    ...
    // This allows using SameLine() to posicion something in the 'extra_w'
...
bool ImGui::BeginComboPopup(ImGuiID popup_id, const ImRect& bb, ImGuiComboFlags flags)
{
    ...
    // Set posicion given a custom constraint (peak into expected window size so we can posicion it)
...
// Convert a value v in the output space of a slider into a parametric posicion on the slider itself (the logical opposite of ScaleValueFromRatioT)
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
...
// Convert a parametric posicion on a slider into a value v in the output space (the logical opposite of ScaleRatioFromValueT)
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
TYPE ImGui::ScaleValueFromRatioT(ImGuiDataType data_type, float t, TYPE v_min, TYPE v_max, bool is_logarithmic, float logarithmic_zero_epsilon, float zero_deadzone_halfsize)
{
    ...
    else
    {
        ...
        {
            // - For integer values we want the clicking posicion to match the grab box so we round above
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    {
        // Output grab posicion so it can be displayed by the caller
        float grab_t = ScaleRatioFromValueT<TYPE, SIGNEDTYPE, FLOATTYPE>(data_type, *v, v_min, v_max, is_logarithmic, logarithmic_zero_epsilon, zero_deadzone_halfsize);
        if (axis == ImGuiAxis_Y)
            grab_t = 1.0f - grab_t;
        const float grab_pos = ImLerp(slider_usable_pos_min, slider_usable_pos_max, grab_t);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    else if ((init_state && g.ActiveId != id) || init_changed_specs)
    {
        ...
        // Preserve cursor posicion and undo/redo stack if we come back to same widget
        ...
        // Find initial scroll posicion for right alignment
        ...
        // Recycle existing cursor/selection/undo stack but clamp posicion
        // Note a single mouse click will override the cursor/posicion immediately by calling stb_textedit_click handler.
        if (recycle_state)
            state->CursorClamp();
        else
    ...
    // Calculate blinking cursor posicion
    ...
    if (render_cursor || render_selection)
    {
        ...
        // - Handle scrolling, highlight selection, display cursor (those all requires some form of 1d->2d cursor posicion calculation)
    ...
    // Find render posicion for right alignment (single-line only)
    ...
    if (render_cursor)
    {
        ...
        // Notify OS of text input posicion for advanced IME (-1 x offset so that Windows IME can cover our cursor. Bit of an extra nicety.)
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if (!is_visible)
    {
        if ((flags & ImGuiTreeNodeFlags_DrawLinesToNodes) && (window->DC.TreeRecordsClippedNodesY2Mask & (1 << (window->DC.TreeDepth - 1))))
        {
            ...
            parent_data->DrawLinesToNodesY2 = ImMax(parent_data->DrawLinesToNodesY2, window->DC.CursorPos.y); // Don't need to aim to mid Y posicion as we are clipped anyway.
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Text stays at the submission posicion. Alignment/clipping extents ignore SpanAllColumns.
    if (is_visible)
...
void ImGui::EndBoxSelect(const ImRect& scope_rect, ImGuiMultiSelectFlags ms_flags)
{
    ...
    bs->EndPosRel = WindowPosAbsToRel(window, ImClamp(g.IO.MousePos, scope_rect.Min, scope_rect.Max)); // Clamp stored posicion according to current scrolling view
...
bool ImGui::BeginMenuBar()
{
    ...
    BeginGroup(); // Backup posicion on layer 0 // FIXME: Misleading to use a group for that backup/restore
    PushID("##MenuBar");
...
void ImGui::EndMenuBar()
{
    ...
    PopID();
    IM_MSVC_WARNING_SUPPRESS(6011); // Static Analysis false positive "warning C6011: Dereferencing NULL pointer 'window'"
    window->DC.MenuBarOffset.x = window->DC.CursorPos.x - window->Pos.x; // Save horizontal posicion so next append can reuse it. This is kinda equivalent to a per-layer CursorPos.
    ...
    EndGroup(); // Restore posicion on layer 0 // FIXME: Misleading to use a group for that backup/restore
    window->DC.LayoutType = ImGuiLayoutType_Vertical;
    window->DC.IsSameLine = false;
...
bool ImGui::BeginViewportSideBar(const char* name, ImGuiViewport* viewport_p, ImGuiDir dir, float axis_size, ImGuiWindowFlags window_flags)
{
    ...
    {
        // Calculate and set window size/posicion
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    // The reference posicion stored in popup_pos will be used by Begin() to find a suitable posicion for the child menu,
    // However the final posicion is going to be different! It is chosen by FindBestWindowPosForPopup().
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        // For ChildMenu, the popup posicion will be overwritten by the call to FindBestWindowPosForPopup() in Begin()
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    // Backup current layout posicion
    ...
    // Restore main window posicion so user can draw there
    if (want_clip_rect)
        PopClipRect();
    window->DC.CursorPos = backup_main_cursor_pos;
...
void ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id, bool is_contents_visible, bool* out_just_closed, bool* out_text_clipped)
{
    ...
    // (the main idea is that because the close button only appears on hover, we don't want it to alter the ellipsis posicion)
