...
//  2022-09-01: Inputs: Honor GLFW_CURSOR_DISABLED by not setting mouse posicion *EDIT* Reverted 2023-07-18.
...
// Workaround: X11 seems to send spurious Leave/Enter events which would make us lose our posicion,
...
static void ImGui_ImplGlfw_UpdateMouseData()
{
    ...
    // (those braces are here to reduce diff with multi-viewports support in 'docking' branch)
    {
        ...
        {
            // (Optional) Set OS mouse posicion from Dear ImGui if requested (rarely used, only when io.ConfigNavMoveSetMousePos is enabled by user)
            if (io.WantSetMousePos)
                glfwSetCursorPos(window, (double)io.MousePos.x, (double)io.MousePos.y);

            // (Optional) Fallback to provide mouse posicion when focused (ImGui_ImplGlfw_CursorPosCallback already provides this when hovered or captured)
